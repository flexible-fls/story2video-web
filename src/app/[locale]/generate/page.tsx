...
export default function GeneratePage() {
  ...
  async function handleGenerate() {
    ...
    if (!text) {
      setErrorMessage(isZh ? "请先输入或粘贴剧本内容" : "Please paste your script first");
      return;
    }
      ...
      <section className="mx-auto max-w-6xl px-6 py-14">
        <div className="mb-8">
          ...
          <p className="mt-3 text-zinc-400">
            {isZh
              ? "这一步会先创建 generation job，再写入结构化结果。后续接入真实 AI 生成时，不需要推翻页面结构。"
              : "This flow creates a generation job first, then writes structured output. You can later replace the demo logic with real AI generation without rebuilding the page."}
          </p>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          ...
          <div className="rounded-3xl border border-white/10 bg-zinc-900 p-6">
            ...
            <div className="mt-6 rounded-2xl bg-zinc-950 p-4 text-sm text-zinc-400">
              {isZh
                ? "说明：当前先用任务系统承接生成流程，后续你接 OpenAI、图片模型、视频模型时，只需要替换中间生成逻辑。"
                : "This page already uses the job system, so later you only need to replace the middle generation logic with real AI services."}
            </div>
          </div>
        </div>
      </section>
